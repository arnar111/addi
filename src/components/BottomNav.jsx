...
import { LayoutDashboard, CheckSquare, Wallet, FileText, Home } from 'lucide-react'

const NAV = [
  { to: '/', icon: LayoutDashboard, label: 'Heim' },
  { to: '/lendo', icon: () => <span style={{ fontSize: 18, lineHeight: 1 }}>🪑</span>, label: 'Lendó' },
      ...
      <div className="flex items-center justify-around px-1 pt-2 pb-1">
        {NAV.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to === '/'}
            ...
            {({ isActive }) => (
              <>
                <div className={`p-1.5 rounded-xl transition-all flex items-center justify-center ${isActive ? 'bg-[rgba(0,212,170,0.15)]' : ''}`}

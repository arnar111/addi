...
import { LayoutDashboard, CheckSquare, Wallet, FileText, Armchair } from 'lucide-react'

const NAV = [
  { to: '/', icon: LayoutDashboard, label: 'Heim' },
  { to: '/lendo', icon: Armchair, label: 'Lendó', highlight: true },
          ...
          <NavLink key={to} to={to} end={to === '/'}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-all ${
                isActive ? 'text-[var(--accent)]' : highlight ? 'text-[#f97316]' : 'text-[var(--muted)]'
              }`
            }>
            {({ isActive }) => (
              <>
                <div className={`p-1.5 rounded-xl transition-all ${
                  isActive ? 'bg-[rgba(0,212,170,0.15)]' : highlight ? 'bg-[rgba(249,115,22,0.1)]' : ''
                }`}>

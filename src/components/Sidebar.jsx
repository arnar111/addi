...
import { LayoutDashboard, CheckSquare, Wallet, FileText, Timer, Settings, Trophy, CreditCard } from 'lucide-react'
...
export default function Sidebar() {
  ...
  const hasAlerts = alertSubs.length > 0
    ...
    <aside className="hidden md:flex flex-col w-56 shrink-0 h-screen sticky top-0 py-6 px-3"
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {NAV.map(({ to, icon: Icon, label }) => {
          const showBadge = to === '/subs' && hasAlerts
          return (
            <NavLink key={to} to={to} end={to === '/'}
              ...
              {({ isActive }) => (
                <>
                  ...
                  {label}
                  {showBadge && (
                    <span className="ml-auto w-4 h-4 rounded-full flex items-center justify-center text-xs font-bold"

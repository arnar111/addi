...
export default function SubscriptionsWidget() {
  const { monthlyISK, failedSubs, activeSubs } = useSubscriptions()
    ...
    <div className="flex flex-col gap-2">
      {failedSubs.length > 0 && (
        <Link to="/subscriptions"
          className="card flex items-center gap-3 py-3 transition-all hover:border-[rgba(239,68,68,0.4)]"
          style={{ border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.05)', textDecoration: 'none' }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold" style={{ color: 'var(--danger)' }}>
              {failedSubs.length} áskrift{failedSubs.length > 1 ? 'ir' : ''} með greiðsluvandamál
            </div>
      ...
      <div className="card">
        ...
        <div className="flex items-end justify-between">
          <div>
            <div className="text-2xl font-semibold">{formatShortISK(monthly)}</div>
            <div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>á mánuði · {activeSubs.length} virkar áskriftir</div>
          </div>
          {failedSubs.length > 0 && (
            <div className="text-right">
              <div className="text-sm font-semibold" style={{ color: 'var(--danger)' }}>{failedSubs.length} bilaðar</div>
              <div className="text-xs" style={{ color: 'var(--muted)' }}>þurfa athygli</div>
            </div>
          )}
        </div>

        {activeSubs.slice(0, 3).length > 0 && (

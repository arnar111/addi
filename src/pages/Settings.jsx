...
const HABIT_COLORS = ['#00d4aa', '#8b5cf6', '#3b82f6', '#f97316', '#22c55e', '#eab308', '#ef4444', '#ec4899']

export default function Settings() {
  const [name, setName] = useLocalStorage('addi_name', 'Arnar')
  const [city, setCity] = useLocalStorage('addi_city', 'Garðabær')
  const { habits, add, remove, isDoneToday, streakFor } = useHabits()
  ...
  const handleAddHabit = (e) => {
    ...
    add(habitName.trim(), habitIcon, habitColor)
    setHabitName('')
    setHabitIcon('⭐')
    setHabitColor('#00d4aa')
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowHabitForm(!showHabitForm)} className="btn btn-ghost text-xs py-1.5 px-3">
        ...
        {showHabitForm && (
          <form onSubmit={handleAddHabit} className="flex flex-col gap-2 p-3 rounded-xl" style={{ background: 'var(--surface2)' }}>
            <div className="flex gap-2">
              <input className="input w-14 text-center text-xl" value={habitIcon} onChange={e => setHabitIcon(e.target.value)} maxLength={2} />
              <input className="input flex-1" placeholder="Nafn vana..." value={habitName} onChange={e => setHabitName(e.target.value)} autoFocus />
            </div>
            <div className="flex gap-2 flex-wrap">
              {HABIT_COLORS.map(c => (
                <button key={c} type="button" onClick={() => setHabitColor(c)}
                  className="w-7 h-7 rounded-full transition-all"
                  style={{ background: c, outline: habitColor === c ? `2px solid white` : 'none', outlineOffset: 2 }} />
              ))}
            </div>
            <div className="flex gap-2">
              <button type="submit" className="btn btn-primary text-xs py-1.5 flex-1 justify-center">Bæta við</button>
        ...
        <div className="flex flex-col gap-2">
          {habits.map(h => {
            ...
            return (
              <div key={h.id} className="flex items-center gap-3 py-2 px-1">
                ...
                {done && <span className="text-xs" style={{ color: 'var(--accent)' }}>✓</span>}

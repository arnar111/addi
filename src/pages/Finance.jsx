...
import { Plus, Trash2, X, TrendingDown, TrendingUp, ArrowUp, ArrowDown } from 'lucide-react'
...
export default function Finance() {
  const {
    ...
    budget, setBudget,
    monthlyTotal, monthlyIncome, byCategory, remaining, savingsRate, netPosition,
  ...
  const isOver = left < 0
  const pct = budget.monthly ? Math.min(100, Math.round((total / budget.monthly) * 100)) : 0
      ...
      <div className="flex items-center justify-between px-1 pt-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="btn"
            style={{ background: 'rgba(34,197,94,0.12)', color: 'var(--success)', border: '1px solid rgba(34,197,94,0.25)', padding: '8px 12px' }}>
            <ArrowUp size={15} /> Tekjur
          </button>
          <button
            onClick={() => { setShowExpenseForm(!showExpenseForm); setShowIncomeForm(false) }}
            className="btn btn-primary">
            <ArrowDown size={15} /> Gjald
          </button>
        </div>
      </div>
        ...
        {income > 0 && (
          <div className="grid grid-cols-2 gap-2 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
            <div className="flex flex-col gap-0.5 p-2.5 rounded-xl" style={{ background: 'rgba(34,197,94,0.08)' }}>
              ...
              <span className="font-semibold text-sm" style={{ color: 'var(--success)' }}>{formatShortISK(income)}</span>
            </div>
            <div className="flex flex-col gap-0.5 p-2.5 rounded-xl" style={{ background: net >= 0 ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)' }}>
              <span className="text-xs" style={{ color: 'var(--muted)' }}>
                {savings !== null ? `Sparnaðarhlutfall` : 'Staða'}
              </span>
              <span className="font-semibold text-sm" style={{ color: net >= 0 ? 'var(--success)' : 'var(--danger)' }}>
                {savings !== null ? `${savings}%` : formatShortISK(net)}
              </span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2">
          {recentExpenses.length === 0 ? (
            <div className="card text-center py-10" style={{ color: 'var(--muted)' }}>
              ...
              <div className="text-sm">Engar færslur ennþá</div>
            </div>
        ...
        <div className="flex flex-col gap-2">
          {income > 0 && (
            <div className="card" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)' }}>
              ...
              <div className="text-2xl font-semibold" style={{ color: 'var(--success)' }}>{formatISK(income)}</div>
              {savings !== null && (
                <div className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
                  Sparnaðarhlutfall: <span style={{ color: savings >= 20 ? 'var(--success)' : savings >= 0 ? '#f97316' : 'var(--danger)' }}>{savings}%</span>
                </div>
              )}
            </div>
          )}
          {recentIncomes.length === 0 ? (
            <div className="card text-center py-10" style={{ color: 'var(--muted)' }}>
              <div className="text-3xl mb-2">💼</div>
              <div className="text-sm">Engar tekjur skráðar ennþá</div>
              <button onClick={() => { setShowIncomeForm(true); setTab('income') }}
                className="btn btn-ghost text-xs mt-3 mx-auto"
                style={{ color: 'var(--success)' }}>
                <Plus size={14} /> Bæta við tekjum
              </button>
            </div>
          ) : recentIncomes.map(i => {
            ...
            return (
              <div key={i.id} className="card flex items-center gap-3 py-3">
                ...
                <button onClick={() => removeIncome(i.id)} style={{ color: 'var(--muted)', padding: 4 }}>
                  <Trash2 size={14} />
                </button>
              </div>
            )

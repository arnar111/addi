...
function SubscriptionsTab() {
  ...
  const handleAdd = (e) => {
    ...
    add({ name: name.trim(), icon, amount })
    setName('')
    setIcon('📱')
    setAmount('')
    setShowForm(false)
  ...
  const quickAdd = (sub) => {
    add({ name: sub.name, icon: sub.icon, amount: sub.amount, color: sub.color })
  }
        ...
        {subs.map(s => (
          <div key={s.id} className="card flex items-center gap-3 py-3"
            ...
            <div className="flex items-center gap-3 shrink-0">
              <ToggleSwitch active={s.active} onToggle={() => toggle(s.id)} />
...
export default function Finance() {
  ...
  const { monthlyTotal: monthlySubTotal } = useSubscriptions()
  const [showForm, setShowForm] = useState(false)

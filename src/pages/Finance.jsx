...
import { Plus, Trash2, X, TrendingDown, TrendingUp, AlertCircle, ToggleLeft, ToggleRight } from 'lucide-react'
        ...
        <div className="flex items-center gap-2">
          ...
          {sub.note && (
            <span className="flex items-center gap-0.5 text-xs" style={{ color: 'var(--danger)' }}>
              <AlertCircle size={10} /> {sub.note}
            </span>
          )}
        </div>
        <div className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>
          Endurnýjast {sub.renewDay}. hvers mánaðar
        </div>
      </div>
...
export default function Finance() {
  ...
  const { subs, add: addSub, toggle: toggleSub, remove: removeSub, monthlyTotal: subTotal, nextRenewals } = useSubscriptions()
  ...
  const [subForm, setSubForm] = useState({ name: '', icon: '📱', amount: '', renewDay: '1', color: '#00d4aa' })
  ...
  const totalSubs = subTotal()
  const renewals = nextRenewals()
  ...
  const handleAddSub = (e) => {
    ...
    addSub(subForm.name, subForm.icon, Number(subForm.amount), 'monthly', Number(subForm.renewDay), subForm.color)
    setSubForm({ name: '', icon: '📱', amount: '', renewDay: '1', color: '#00d4aa' })
  ...
  return (
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(0,212,170,0.06), rgba(139,92,246,0.06))' }}>
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex flex-col items-end gap-2">
            <div>
              <div className="text-xs mb-0.5 text-right" style={{ color: 'var(--muted)' }}>Eftir</div>
            ...
            <div className="text-right">
              ...
              <div className="text-sm font-semibold" style={{ color: '#f97316' }}>{formatShortISK(totalSubs)}</div>
            </div>
          </div>
        </div>
      ...
      {showSubForm && tab === 'subscriptions' && (
        <form onSubmit={handleAddSub} className="card flex flex-col gap-3 animate-slide-up">
          ...
          <div className="grid grid-cols-4 gap-2">
            <input className="input text-sm col-span-1" placeholder="📱" value={subForm.icon}
              onChange={e => setSubForm(f => ({ ...f, icon: e.target.value }))} />
            <input className="input text-sm col-span-3" placeholder="Nafn áskriftar *" value={subForm.name}
              onChange={e => setSubForm(f => ({ ...f, name: e.target.value }))} autoFocus />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <input className="input text-sm" type="number" placeholder="Upphæð/mán (ISK)" value={subForm.amount}
              onChange={e => setSubForm(f => ({ ...f, amount: e.target.value }))} />
            <input className="input text-sm" type="number" min="1" max="31" placeholder="Endurnýjunardagur" value={subForm.renewDay}
              onChange={e => setSubForm(f => ({ ...f, renewDay: e.target.value }))} />
          </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
        {[['overview', 'Yfirlit'], ['subscriptions', '📱 Áskriftir'], ['transactions', 'Færslur']].map(([t, l]) => (
      ...
      {tab === 'subscriptions' && (
        <div className="flex flex-col gap-3">
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(249,115,22,0.08), rgba(139,92,246,0.08))' }}>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs mb-1" style={{ color: 'var(--muted)' }}>Samtals á mánuði</div>
                <div className="text-3xl font-bold" style={{ color: '#f97316' }}>{formatISK(totalSubs)}</div>
                <div className="text-xs mt-1" style={{ color: 'var(--muted)' }}>
                  ≈ {formatISK(totalSubs * 12)} á ári
                </div>
              </div>
          ...
          {renewals.length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-sm mb-3">⏰ Endurnýjast fljótlega</h3>
              <div className="flex flex-col gap-2">
                {renewals.map(s => (
                  <div key={s.id} className="flex items-center gap-3">
                    <span className="text-base">{s.icon}</span>
                    <span className="text-sm flex-1">{s.name}</span>
                    <span className="text-xs" style={{ color: s.daysUntil <= 3 ? 'var(--danger)' : 'var(--muted)' }}>
                      {s.daysUntil === 0 ? 'Í dag!' : `${s.daysUntil} dagar`}
                    </span>
          ...
          {/* Sub list */}
          <div className="card">
            <h3 className="font-semibold text-sm mb-3">Allar áskriftir</h3>
            <div className="flex flex-col gap-2">
              {subs.map(s => (

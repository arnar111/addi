  ...
  return (
    <div className="flex items-center gap-3 py-2.5" style={{ borderBottom: '1px solid var(--border)' }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs" style={{ color: sub.active ? sub.color : 'var(--muted)' }}>
          {formatISK(sub.amount)}/mán · {sub.active ? 'Virk' : 'Óvirk'}
        </div>
      </div>
      <button onClick={() => onToggle(sub.id)} className="shrink-0">
        {sub.active
...
export default function Finance() {
  ...
  const { subs, add: addSub, remove: removeSub, toggle: toggleSub, monthlyTotal: subTotal, activeSubs } = useSubscriptions()
  ...
  const handleAddSub = (e) => {
    ...
    addSub({ name: subName, icon: subIcon, color: '#00d4aa', amount: Number(subAmount), billing: 'monthly', active: true })
    setSubName('')
    setSubAmount('')
    setSubIcon('📱')
  ...
  const TABS = [
    ...
    ['subscriptions', `Áskriftir (${activeSubs.length})`],
  ]
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(0,212,170,0.06), rgba(139,92,246,0.06))' }}>
        ...
        <div className="mt-3 pt-3 flex items-center justify-between text-xs"
          ...
          <span style={{ color: 'var(--muted)' }}>📱 Áskriftir ({activeSubs.length})</span>
          <span style={{ color: 'var(--accent)' }}>{formatISK(subTotal)}/mán</span>
        </div>
      </div>
      ...
      {showSubForm && (
        <form onSubmit={handleAddSub} className="card flex flex-col gap-3 animate-slide-up">
          ...
          <div className="flex gap-2">
            <input className="input text-2xl text-center" style={{ maxWidth: 60 }} value={subIcon}
              onChange={e => setSubIcon(e.target.value)} placeholder="📱" />
            <input className="input flex-1" placeholder="Heiti (t.d. HBO Max)" value={subName}
              onChange={e => setSubName(e.target.value)} autoFocus />
          </div>
          <input className="input" type="number" placeholder="Upphæð á mánuði (ISK)" value={subAmount}
      ...
      {tab === 'subscriptions' && (
        <div className="card flex flex-col">
          <div className="flex items-center justify-between mb-2 pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
            <span className="text-sm font-semibold">Mánaðarlegar áskriftir</span>
            <span className="font-bold" style={{ color: 'var(--accent)' }}>{formatISK(subTotal)}</span>
          </div>
          {subs.map(sub => (
            <SubscriptionRow key={sub.id} sub={sub} onToggle={toggleSub} onRemove={removeSub} />
          ))}
          {subs.length === 0 && (
          ...
          <div className="pt-3 mt-1 text-xs" style={{ color: 'var(--muted)', borderTop: '1px solid var(--border)' }}>
            Áskriftir teljast ekki sjálfkrafa inn í mánaðarleg útgjöld — skrá þær sér.
          </div>
        </div>
      )}
    </div>

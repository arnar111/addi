...
const INCOME_SOURCES = [
  ...
  { id: 'freelance', label: 'Verktöku', icon: '💻', color: '#8b5cf6' },
...
export default function Finance() {
  const {
    ...
    budget, setBudget, monthlyTotal, monthlyIncome, monthlyNet,
    byCategory, remaining, recentExpenses, recentIncome,
  } = useFinance()
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(0,212,170,0.06), rgba(139,92,246,0.06))' }}>
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-lg font-semibold" style={{ color: 'var(--success)' }}>{formatShortISK(income)}</span>
          </div>
          <div className="flex flex-col gap-0.5 items-center">
            <span className="text-xs" style={{ color: 'var(--muted)' }}>Útgjöld</span>
            <span className="text-lg font-semibold">{formatShortISK(total)}</span>
          </div>
          <div className="flex flex-col gap-0.5 items-end">
            <span className="text-xs" style={{ color: 'var(--muted)' }}>Nettó</span>
            <span className="text-lg font-semibold" style={{ color: net >= 0 ? 'var(--success)' : 'var(--danger)' }}>
      ...
      {showIncomeForm && (
        <form onSubmit={handleAddIncome} className="card flex flex-col gap-3 animate-slide-up"
          ...
          <input className="input" placeholder="Lýsing (valkvæmt)" value={incomeNote}
      ...
      {/* Tabs */}
      <div className="flex gap-2">
        {[['overview', 'Yfirlit'], ['transactions', 'Útgjöld'], ['income_tab', 'Tekjur']].map(([t, l]) => (
      ...
      {tab === 'transactions' && (
        <div className="flex flex-col gap-2">
          {recentExpenses.length === 0 ? (
            <div className="card text-center py-8" style={{ color: 'var(--muted)' }}>Engar útgjaldsfærslur ennþá</div>

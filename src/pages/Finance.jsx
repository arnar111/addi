...
export default function Finance() {
  ...
  const { income: incomeList, add: addIncome, remove: removeIncome, monthlyTotal: monthlyIncome, monthlyGoal, setMonthlyGoal } = useIncome()
  ...
  const netMonth = income - total
  const incomePct = Math.min(100, Math.round((income / monthlyGoal) * 100))

  const handleAddIncome = (e) => {
    e.preventDefault()
    if (!incomeAmount || isNaN(Number(incomeAmount))) return
    addIncome(Number(incomeAmount), 'lendo', incomeNote)
    setIncomeAmount('')
    setIncomeNote('')
  ...
  return (
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="flex gap-3">
        <div className="flex-1 p-3 rounded-xl" style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.15)' }}>
        ...
        <div className="flex-1 p-3 rounded-xl" style={{ background: netMonth >= 0 ? 'rgba(0,212,170,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${netMonth >= 0 ? 'rgba(0,212,170,0.15)' : 'rgba(239,68,68,0.15)'}` }}>
          <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Nettó</div>
          <div className="text-lg font-bold" style={{ color: netMonth >= 0 ? 'var(--accent)' : 'var(--danger)' }}>
            {netMonth >= 0 ? '+' : ''}{formatShortISK(netMonth)}
          </div>
        </div>
      </div>
      ...
      {tab === 'income' && (
        <div className="flex flex-col gap-4">
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.08), rgba(0,212,170,0.06))' }}>
            <div className="flex justify-between items-start mb-3">
              <div>
                ...
                <div className="text-3xl font-semibold" style={{ color: 'var(--success)' }}>{formatISK(income)}</div>
              </div>
              <div>
                <div className="text-xs mb-1 text-right" style={{ color: 'var(--muted)' }}>Markmið</div>
                <div className="text-lg font-semibold text-right">{incomePct}%</div>
              </div>
            </div>
            <div className="h-2.5 rounded-full overflow-hidden mb-1" style={{ background: 'var(--surface2)' }}>
              <div className="h-full rounded-full transition-all"
                   style={{ width: `${incomePct}%`, background: incomePct >= 100 ? 'var(--success)' : incomePct >= 50 ? 'var(--accent)' : '#f97316' }} />
            </div>
            <div className="flex justify-between text-xs" style={{ color: 'var(--muted)' }}>
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold">Tekjulisti</span>
            <button onClick={() => setShowIncomeForm(s => !s)} className="btn btn-primary text-sm">
          ...
          {showIncomeForm && (
            <form onSubmit={handleAddIncome} className="card flex flex-col gap-3 animate-slide-up">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-sm">Skrá Lendó tekjur</h3>
              ...
              <input className="input" placeholder="Lýsing (t.d. Cat leiga)" value={incomeNote}
              ...
              {incomeList.slice(0, 20).map(i => (
                <div key={i.id} className="card flex items-center gap-3 py-3">
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
                       style={{ background: 'rgba(34,197,94,0.15)', fontSize: 16 }}>🏠</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium" style={{ color: 'var(--success)' }}>+{formatISK(i.amount)}</div>
                    <div className="text-xs" style={{ color: 'var(--muted)' }}>
                      {i.note || 'Lendó leiga'} · {new Date(i.date).toLocaleDateString('is-IS', { month: 'short', day: 'numeric' })}
                    </div>
                  </div>
          ...
          <div className="card">
            <div className="text-xs font-semibold mb-2" style={{ color: 'var(--muted)' }}>Breyta tekjumarkmiði</div>
            <input className="input text-sm" type="number" placeholder="Mánaðarlegt tekjumarkmið (ISK)"
                   value={monthlyGoal}

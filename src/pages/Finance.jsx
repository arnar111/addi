...
export default function Finance() {
  ...
  const { subs, add: addSub, remove: removeSub, monthlyTotal: subMonthly, yearlyTotal: subYearly } = useSubscriptions()
  ...
  const handleAddSub = (e) => {
    ...
    addSub({ name: subName.trim(), amount: Number(subAmount), cycle: subCycle, category: subCategory })
    setSubName('')
    setSubAmount('')
    setSubCycle('monthly')
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="flex gap-2">
        {[['overview', 'Yfirlit'], ['transactions', 'Færslur'], ['subscriptions', 'Áskriftir']].map(([t, l]) => (
          <button
            key={t}
            onClick={() => { setTab(t); setShowForm(false); setShowSubForm(false) }}
      ...
      {tab === 'subscriptions' && (
        <div className="flex flex-col gap-3">
          ...
          <div
            className="card"
            style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(0,212,170,0.04))' }}
          >
            <div className="flex justify-between items-center">
              <div>
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Mánaðarlegt</div>
                <div className="text-2xl font-semibold">{formatISK(subMonthly)}</div>
              </div>
              <div className="text-right">
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Árlegt</div>
                <div className="text-xl font-semibold" style={{ color: 'var(--accent2)' }}>{formatISK(subYearly)}</div>
              </div>
            </div>
          </div>
          ...
          {showSubForm && (
            <form onSubmit={handleAddSub} className="card flex flex-col gap-3 animate-slide-up">
              ...
              />
              <div className="flex gap-2">
                {[['monthly', 'Mánaðarlegt'], ['yearly', 'Árlegt']].map(([c, l]) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => setSubCycle(c)}
                    className="flex-1 py-2 rounded-xl text-xs font-medium transition-all"
                    style={{
                      background: subCycle === c ? 'rgba(0,212,170,0.12)' : 'var(--surface2)',
                      color: subCycle === c ? 'var(--accent)' : 'var(--muted)',
                      border: `1px solid ${subCycle === c ? 'rgba(0,212,170,0.3)' : 'transparent'}`,
                    }}
                  >
                    {l}
                  </button>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {SUB_CATEGORIES.map(c => (
                  <button
                    ...
                    className="flex flex-col items-center gap-0.5 py-2 rounded-xl text-xs transition-all"
                    style={{
                      background: subCategory === c.id ? 'rgba(0,212,170,0.12)' : 'var(--surface2)',
                      border: `1px solid ${subCategory === c.id ? 'rgba(0,212,170,0.3)' : 'transparent'}`,
                    }}
                  >
                    <span>{c.icon}</span>
                    <span style={{ color: subCategory === c.id ? 'var(--accent)' : 'var(--muted)', fontSize: 10 }}>
                      {c.label}
                    </span>
                  </button>
                ))}
          ...
          {subs.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {subs.map(s => {
                const cat = SUB_CATEGORIES.find(c => c.id === s.category) || SUB_CATEGORIES[7]
                ...
                return (
                  <div key={s.id} className="card flex items-center gap-3 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs" style={{ color: 'var(--muted)' }}>
                        {cat.label} · {s.cycle === 'yearly' ? 'Árlegt' : 'Mánaðarlegt'}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-sm font-semibold">{formatShortISK(s.amount)}</div>
                      {s.cycle === 'yearly' && (
                        <div className="text-xs" style={{ color: 'var(--muted)' }}>
                          {formatShortISK(monthly)}/mán
                        </div>
                      )}
                    </div>
                    <button onClick={() => removeSub(s.id)} style={{ color: 'var(--muted)', marginLeft: 4 }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                )

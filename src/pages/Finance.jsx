...
import { Plus, Trash2, X, TrendingDown, TrendingUp, ArrowDownCircle, ArrowUpCircle, PiggyBank } from 'lucide-react'

const INCOME_SOURCES = [
  { id: 'salary', label: 'Laun', icon: '💼' },
  { id: 'freelance', label: 'Sjálfstætt', icon: '💻' },
  { id: 'investment', label: 'Fjárfesting', icon: '📈' },
...
export default function Finance() {
  const {
    ...
    budget, setBudget, monthlyTotal, monthlyIncome, monthlySavings,
  ...
  const savingsPct = budget.savingsGoal ? Math.min(100, Math.round((savings / budget.savingsGoal) * 100)) : 0
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      <div className="flex items-center justify-between px-1 pt-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setShowIncomeForm(!showIncomeForm); setShowExpenseForm(false) }} className="btn btn-ghost text-xs">
            <ArrowDownCircle size={14} style={{ color: 'var(--success)' }} /> Tekjur
          </button>
          <button onClick={() => { setShowExpenseForm(!showExpenseForm); setShowIncomeForm(false) }} className="btn btn-primary text-xs">
            <ArrowUpCircle size={14} /> Gjald
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="card-sm flex flex-col gap-0.5">
          ...
          <div className="text-base font-semibold" style={{ color: 'var(--success)' }}>{formatShortISK(income)}</div>
        </div>
        <div className="card-sm flex flex-col gap-0.5">
        ...
        <div className="card-sm flex flex-col gap-0.5">
          ...
          <div className="text-base font-semibold" style={{ color: savings >= 0 ? 'var(--accent)' : 'var(--danger)' }}>{formatShortISK(savings)}</div>
        </div>
      </div>
        ...
        {budget.savingsGoal > 0 && income > 0 && (
          <div className="mt-3 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
            <div className="flex justify-between text-xs mb-1.5" style={{ color: 'var(--muted)' }}>
              <span className="flex items-center gap-1"><PiggyBank size={11} /> Sparnaðarmarkmið</span>
              <span>{formatShortISK(savings)} / {formatShortISK(budget.savingsGoal)}</span>
            </div>
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--surface2)' }}>
              <div className="h-full rounded-full transition-all"
                   style={{ width: `${Math.max(0, savingsPct)}%`, background: savings >= budget.savingsGoal ? 'var(--success)' : 'var(--accent2)' }} />
            </div>
          </div>
        )}
      </div>
      ...
      {showIncomeForm && (
        <form onSubmit={handleAddIncome} className="card flex flex-col gap-3 animate-slide-up"
          ...
          <input className="input" type="number" placeholder="Upphæð (ISK)" value={incomeAmount} onChange={e => setIncomeAmount(e.target.value)} autoFocus />
          <input className="input" placeholder="Skýring (valkvæmt)" value={incomeNote} onChange={e => setIncomeNote(e.target.value)} />
          <div className="grid grid-cols-4 gap-1.5">
            {INCOME_SOURCES.map(s => (
              <button key={s.id} type="button" onClick={() => setIncomeSource(s.id)}
                className="flex flex-col items-center gap-0.5 py-2 rounded-xl text-xs transition-all"
                style={{
                  background: incomeSource === s.id ? 'rgba(34,197,94,0.15)' : 'var(--surface2)',
          ...
          <button type="submit" className="btn w-full justify-center" style={{ background: 'var(--success)', color: '#000' }}>Bæta við</button>
        </form>
      )}
            ...
            <div className="flex flex-col gap-2 p-3 rounded-xl" style={{ background: 'var(--surface2)' }}>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex flex-col gap-0.5">
                  <label className="text-xs" style={{ color: 'var(--muted)' }}>Mánaðarleg áætlun</label>
                  <input className="input text-xs py-1.5" type="number"
                    value={budget.monthly}
                    onChange={e => setBudget(b => ({ ...b, monthly: Number(e.target.value) }))} />
                </div>
                <div className="flex flex-col gap-0.5">
                  <label className="text-xs flex items-center gap-1" style={{ color: 'var(--muted)' }}><PiggyBank size={11} /> Sparnaðarmark</label>
                  <input className="input text-xs py-1.5" type="number"

...
export default function Finance() {
  ...
  const { subs, add: addSub, remove: removeSub, toggleActive, monthlyTotal: monthlySubTotal } = useSubscriptions()
  ...
  const handleAddSub = (e) => {
    ...
    addSub(subName.trim(), subAmount, subPeriod, subIcon)
    setSubName('')
    setSubAmount('')
      ...
      {tab === 'subscriptions' && (
        <div className="flex flex-col gap-4">
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(0,212,170,0.06))' }}>
            ...
            <div className="text-3xl font-semibold">{formatISK(Math.round(monthlySubTotal))}</div>
            <div className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
              {formatISK(Math.round(monthlySubTotal * 12))} á ári
            </div>
          </div>
          ...
          {showSubForm && (
            <form onSubmit={handleAddSub} className="card flex flex-col gap-3 animate-slide-up">
              ...
              />
              <div className="flex gap-2">
                {[['monthly', 'Á mánuði'], ['yearly', 'Á ári']].map(([v, l]) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => setSubPeriod(v)}
                    className="flex-1 py-1.5 rounded-lg text-xs font-medium transition-all"
                    style={{
                      background: subPeriod === v ? 'rgba(139,92,246,0.2)' : 'var(--surface2)',
                      color: subPeriod === v ? '#8b5cf6' : 'var(--muted)',
                      border: `1px solid ${subPeriod === v ? '#8b5cf669' : 'transparent'}`,
                    }}
                  >
                    {l}
                  </button>
          ...
          {/* Quick-add popular */}
          {POPULAR_SUBS.filter(p => !subs.some(s => s.name === p.name)).length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-sm mb-3">Vinsælar áskriftir</h3>
              <div className="flex flex-wrap gap-2">
                {POPULAR_SUBS.filter(p => !subs.some(s => s.name === p.name)).map(p => (
                  <button
                    key={p.name}
                    onClick={() => addSub(p.name, p.amount, p.period, p.icon)}
          ...
          <div className="flex flex-col gap-2">
            {subs.length === 0 ? (
              <div className="card text-center py-8" style={{ color: 'var(--muted)' }}>
                Engar áskriftir ennþá — bættu við hér að ofan
              </div>
            ) : subs.map(s => (
              <div
                key={s.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs" style={{ color: 'var(--muted)' }}>
                    {formatISK(s.amount)} {s.period === 'monthly' ? 'á mánuði' : 'á ári'}
                    {s.period === 'yearly' && ` · ${formatISK(Math.round(s.amount / 12))}/mán`}
                  </div>
                </div>
                <ToggleSwitch active={s.active} onToggle={() => toggleActive(s.id)} />

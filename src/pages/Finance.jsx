...
function SubCard({ sub, onToggle, onRemove, monthlyAmount }) {
  const cat = SUB_CATEGORIES.find(c => c.id === sub.category) || SUB_CATEGORIES[6]
  return (
    <div className="card flex items-center gap-3 py-3" style={{ opacity: sub.active ? 1 : 0.55 }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs truncate" style={{ color: 'var(--muted)' }}>
          {sub.note}
          {!sub.active && <span style={{ color: 'var(--danger)' }}> · Í bið</span>}
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <div className="text-right">
          <div className="text-sm font-semibold">{formatShortISK(monthlyAmount(sub))}</div>
...
export default function Finance() {
  ...
  const { subs, add: addSub, remove: removeSub, toggle: toggleSub, monthlyTotal: subMonthly, annualTotal, monthlyAmount } = useSubscriptions()
  ...
  const [subForm, setSubForm] = useState({ name: '', icon: '📦', category: 'other', amount: '', cycle: 'monthly', note: '' })
  ...
  const handleAddSub = (e) => {
    ...
    addSub({ ...subForm, amount: Number(subForm.amount), active: true })
    setSubForm({ name: '', icon: '📦', category: 'other', amount: '', cycle: 'monthly', note: '' })
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(0,212,170,0.06), rgba(139,92,246,0.06))' }}>
        ...
        <div className="mt-3 pt-3 flex justify-between text-xs" style={{ borderTop: '1px solid var(--border)' }}>
          <span style={{ color: 'var(--muted)' }}>📱 Áskriftir/mán</span>
          <span style={{ color: 'var(--accent)', fontWeight: 600 }}>{formatShortISK(subMonthly())}</span>
        </div>
      </div>
      ...
      {showSubForm && tab === 'subscriptions' && (
        <form onSubmit={handleAddSub} className="card flex flex-col gap-3 animate-slide-up">
          ...
          <div className="grid grid-cols-4 gap-2">
            <input className="input text-center text-xl" placeholder="📦" value={subForm.icon}
              onChange={e => setSubForm(f => ({ ...f, icon: e.target.value }))} style={{ gridColumn: 'span 1' }} />
            <input className="input text-sm" placeholder="Nafn áskriftar" value={subForm.name}
              onChange={e => setSubForm(f => ({ ...f, name: e.target.value }))} style={{ gridColumn: 'span 3' }} autoFocus />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <input className="input text-sm" type="number" placeholder="Upphæð (ISK)" value={subForm.amount}
              onChange={e => setSubForm(f => ({ ...f, amount: e.target.value }))} />
            <select className="input text-sm" value={subForm.cycle} onChange={e => setSubForm(f => ({ ...f, cycle: e.target.value }))}>
              <option value="monthly">Mánaðarlega</option>
              <option value="annual">Árlega</option>
              <option value="custom">Annað</option>
            </select>
          </div>
          <input className="input text-sm" placeholder="Skýring (valkvæmt)" value={subForm.note}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(0,212,170,0.06))' }}>
            <div className="flex justify-between items-center mb-2">
              <div>
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Virkar áskriftir/mán</div>
                <div className="text-2xl font-semibold">{formatISK(subMonthly())}</div>
              </div>
              <div className="text-right">
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Á ári</div>
                <div className="text-lg font-semibold" style={{ color: 'var(--accent2)' }}>{formatISK(annualTotal())}</div>
              </div>
            </div>
            <div className="text-xs" style={{ color: 'var(--muted)' }}>
              {subs.filter(s => s.active).length} virkar · {subs.filter(s => !s.active).length} í bið
            </div>
          </div>

          {/* Active */}
          <div className="text-xs font-semibold px-1" style={{ color: 'var(--muted)' }}>VIRKAR</div>
          {subs.filter(s => s.active).map(s => (
            <SubCard key={s.id} sub={s} onToggle={toggleSub} onRemove={removeSub} monthlyAmount={monthlyAmount} />
          ))}

          {/* Paused */}
          {subs.filter(s => !s.active).length > 0 && (
            <>
              <div className="text-xs font-semibold px-1 mt-1" style={{ color: 'var(--muted)' }}>Í BIÐ / VANDRÆÐI</div>
              {subs.filter(s => !s.active).map(s => (
                <SubCard key={s.id} sub={s} onToggle={toggleSub} onRemove={removeSub} monthlyAmount={monthlyAmount} />
              ))}
            </>

...
import { Plus, Trash2, X, TrendingDown, TrendingUp, PiggyBank } from 'lucide-react'
...
export default function Finance() {
  const {
    ...
    addIncome, removeIncome, income, monthlyIncome, monthlySavings,
  } = useFinance()
  ...
  const isOver = left < 0
  const pct = budget.monthly > 0 ? Math.min(100, Math.round((total / budget.monthly) * 100)) : 0
  ...
  const handleAddIncome = (e) => {
    ...
    addIncome(Number(incomeAmount), incomeNote)
    setIncomeAmount('')
    setIncomeNote('')
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="card" style={{ background: 'linear-gradient(135deg, rgba(0,212,170,0.06), rgba(139,92,246,0.06))' }}>
        {incomeTotal > 0 && (
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="flex flex-col gap-0.5 p-2.5 rounded-xl" style={{ background: 'rgba(34,197,94,0.1)' }}>
              <span className="text-xs" style={{ color: 'var(--muted)' }}>Tekjur</span>
              <span className="font-semibold text-sm" style={{ color: 'var(--success)' }}>{formatShortISK(incomeTotal)}</span>
            </div>
            ...
            <div className="flex flex-col gap-0.5 p-2.5 rounded-xl"
                 style={{ background: savings >= 0 ? 'rgba(0,212,170,0.1)' : 'rgba(239,68,68,0.08)' }}>
              <span className="text-xs" style={{ color: 'var(--muted)' }}>Sparnaður</span>
              <span className="font-semibold text-sm"
                    style={{ color: savings >= 0 ? 'var(--accent)' : 'var(--danger)' }}>
      ...
      {showIncomeForm && (
        <form onSubmit={handleAddIncome} className="card flex flex-col gap-3 animate-slide-up"
          ...
          <input className="input" placeholder="Skýring (t.d. Laun, hlutavinna...)" value={incomeNote}
      ...
      {/* Tabs */}
      <div className="flex gap-2">
        {[['overview', 'Yfirlit'], ['transactions', 'Gjöld'], ['income-tab', 'Tekjur']].map(([t, l]) => (
        ...
        <div className="flex flex-col gap-2">
          {income.length === 0 ? (
            <div className="card text-center py-8 flex flex-col items-center gap-2">
              <PiggyBank size={32} style={{ color: 'var(--muted)' }} />
              <div style={{ color: 'var(--muted)' }}>Engar tekjur skráðar ennþá</div>
            </div>
          ) : income.map(i => (
            <div key={i.id} className="card flex items-center gap-3 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs" style={{ color: 'var(--muted)' }}>
                  {i.note || 'Tekjur'} · {new Date(i.date).toLocaleDateString('is-IS', { month: 'short', day: 'numeric' })}
                </div>
              </div>

...
import { Plus, Trash2, X, TrendingUp, TrendingDown, ArrowUpRight, ArrowDownRight } from 'lucide-react'
...
function IncomeBar({ src, earned }) {
  return (
    <div className="flex items-center justify-between text-sm py-1">
      ...
      <span className="font-medium" style={{ color: 'var(--success)' }}>{formatISK(earned)}</span>
    </div>
  )
}
...
export default function Finance() {
  ...
  const {
    addIncome, removeIncome, monthlyGoal, setMonthlyGoal,
    monthlyTotal: monthlyIncome, bySource, recentIncomes,
  } = useIncome()
  ...
  const incGoalPct = Math.min(100, Math.round((income / monthlyGoal) * 100))
    ...
    <div className="flex flex-col gap-4 pb-4 animate-slide-up">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="card-sm flex flex-col gap-0.5"
          ...
          <div className="text-[10px]" style={{ color: 'var(--muted)' }}>{incGoalPct}% af marki</div>
        </div>
        <div className="card-sm flex flex-col gap-0.5"
          ...
          <div className="text-[10px]" style={{ color: 'var(--muted)' }}>{pct}% af áætlun</div>
        </div>
        <div className="card-sm flex flex-col gap-0.5"
             style={{
               background: net >= 0 ? 'rgba(0,212,170,0.06)' : 'rgba(239,68,68,0.06)',
               border: `1px solid ${net >= 0 ? 'rgba(0,212,170,0.15)' : 'rgba(239,68,68,0.15)'}`,
             }}>
          ...
          <div className="text-[10px]" style={{ color: 'var(--muted)' }}>þessi mánuður</div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-2">
        {[['overview', 'Yfirlit'], ['income', '💚 Tekjur'], ['expenses', 'Útgjöld']].map(([t, l]) => (
      ...
      {mainTab === 'overview' && (
        <div className="flex flex-col gap-3">
          {/* Income progress toward goal */}
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.06), rgba(0,212,170,0.06))' }}>
            <div className="flex justify-between items-start mb-3">
              <div>
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Tekjumarkmið þessa mánaðar</div>
                <div className="text-2xl font-bold" style={{ color: 'var(--success)' }}>{formatISK(income)}</div>
              </div>
              <div className="text-right">
                <div className="text-xs mb-0.5" style={{ color: 'var(--muted)' }}>Mark</div>
                <div className="text-base font-semibold">{formatShortISK(monthlyGoal)}</div>
              </div>
            </div>
            <div className="h-2 rounded-full overflow-hidden mb-1" style={{ background: 'var(--surface2)' }}>
              <div className="h-full rounded-full transition-all"
                   style={{ width: `${incGoalPct}%`, background: incGoalPct >= 100 ? 'var(--accent)' : 'var(--success)' }} />
            </div>
            <div className="flex justify-between text-xs" style={{ color: 'var(--muted)' }}>
              <span>{incGoalPct}% af marki</span>
              <span>{formatISK(Math.max(0, monthlyGoal - income))} eftir</span>
            </div>
          </div>
      ...
      {mainTab === 'income' && (
        <div className="flex flex-col gap-3">
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.06), rgba(0,212,170,0.06))' }}>
            <div className="text-xs mb-1" style={{ color: 'var(--muted)' }}>Heildartekjur í mánuð</div>
            <div className="text-3xl font-bold mb-3" style={{ color: 'var(--success)' }}>{formatISK(income)}</div>
            <div className="h-2 rounded-full overflow-hidden mb-2" style={{ background: 'var(--surface2)' }}>
              <div className="h-full rounded-full" style={{ width: `${incGoalPct}%`, background: 'var(--success)' }} />
            </div>
            <div className="flex justify-between text-xs" style={{ color: 'var(--muted)' }}>
              <span>Mark: {formatISK(monthlyGoal)}</span>
              <span>{incGoalPct}%</span>
            </div>
          </div>

          {/* By source */}
          <div className="card flex flex-col gap-2">
            <div className="text-sm font-semibold mb-1">Eftir uppruna</div>
            {INCOME_SOURCES.map(s => (
              sources[s.id] > 0 ? <IncomeBar key={s.id} src={s} earned={sources[s.id] || 0} /> : null
            ))}
